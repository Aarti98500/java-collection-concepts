/**
 * Stack is data structure that is built using array.
...
public class Stack {
    ...
    int top;//this variable/pointer points to top element of th stack.
    ...
    void push(int item){
        //push item into top of the stack.
    ...
    int pop(){
        ...
        //before removing an item from stack is empty or not.
        if (isEmpty()){
            System.out.println("Stack is empty, cannot be removed an item from stack");
            return -1;
        }else {
            int topElement = stack[top];
            System.out.println("Item " + topElement + " popped from stack at position " + top);
            stack[top] = 0;
            top--;
            printStack();
            return topElement;
